#!/bin/bash
# qap/regenerate_all_html.sh — Regenera TODOS los HTMLs en gh-pages con los MDs actuales.
#
...
#
# Uso:
#     ./qap/regenerate_all_html.sh           # regenera TODOS los runs
#     ./qap/regenerate_all_html.sh --last 5  # solo los 5 más recientes (más rápido)
...
for TS in $DIRS; do
  ...
  if python qap/regenerate_html.py --logs-dir "$LOGS_DIR" --out "$TMP_HTML" >/dev/null 2>&1; then
...
python3 qap/rebuild_history.py --out "$TMP_DIR/qa/history.json" 2>&1 | tail -2
