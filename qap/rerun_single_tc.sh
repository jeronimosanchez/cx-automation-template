#!/bin/bash
# qap/rerun_single_tc.sh — Re-ejecuta UN solo TC y publica un nuevo run en gh-pages
#
...
#
# Uso:
#     ./qap/rerun_single_tc.sh <TC-ID> [<BASE_TS>]
#
# Ejemplos:
#     ./qap/rerun_single_tc.sh TC-URGENCIA-01
#     ./qap/rerun_single_tc.sh TC-URGENCIA-01 20260519_115033
#
...
.venv/bin/python qap/regenerate_html.py --logs-dir "$TMP_LOGS" --out "$TMP_HTML" 2>&1 | grep -E "Cargados|Generando|OK" | head -3

# Extraer stats del HTML generado (mismo formato que regenerate_html imprime)
STATS=$(.venv/bin/python qap/regenerate_html.py --logs-dir "$TMP_LOGS" --out "$TMP_HTML" 2>&1 | grep -oE "Cargados: [0-9]+ TCs \([0-9]+ PASS, [0-9]+ INESTABLE, [0-9]+ FAIL\)" | head -1)
