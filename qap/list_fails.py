#!/usr/bin/env python3
"""
qap/list_fails.py — Lista los FAILs de un run y su estado de análisis manual.

Útil para saber qué TCs necesitan análisis. Usado por la skill /qa-tc-analyzer
en modo batch.

Uso:
    python qap/list_fails.py                     # último run de gh-pages
    python qap/list_fails.py --ts 20260518_192907 # run específico
    python qap/list_fails.py --logs-dir PATH      # carpeta local
    python qap/list_fails.py --only-pending       # solo FAILs sin .md
"""
import argparse
import json
import sys
