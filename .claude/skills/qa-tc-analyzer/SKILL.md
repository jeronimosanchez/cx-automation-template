...
Analiza uno o varios TCs fallidos del QA de Petal, escribe un `.md` de causa raíz por TC (9 capas + soluciones puntuadas), detecta patrones cruzados entre ellos, regenera el dashboard HTML y lo publica. **No toca el agente.** Coste 0€.

> **Cómo está organizada esta skill:**
> **§1 Propósito e invocación** · **§2 Qué produce** (los formatos de salida) · **§3 Referencia: las 9 capas y el scoring** · **§4 Flujo de ejecución** (los pasos) · **§5 Reglas, coste y ejemplos**.
> El flujo (§4) referencia los formatos de §2 y la referencia de §3 en lugar de repetirlos.

---

## 1. Propósito e invocación
...
- **INDIVIDUAL** — el usuario nombra un TC concreto. Se analiza inline. Se omite la fase de patrones (§4 Paso 3c).
- **BATCH** — el usuario dice "todos"/"all" o no nombra TC. Un sub-agente por TC en paralelo + un sub-agente de patrones con contexto limpio.

---

## 2. Qué produce (la salida)

Esta skill genera tres artefactos. Verlos primero hace legible el flujo de §4.

### 2.1 El MD de análisis por TC — `qap/tc_analysis/{TS}/{TC-ID}.md`

Estructura EXACTA (el contenido de las 9 capas y el dimensionamiento están en §3):
...
<Emitir las 9 capas OBLIGATORIAMENTE, en orden, con el formato vertical jerárquico. El "Cubre" de cada capa está en §3.1; las reglas de marca en §3.2. Ejemplo del formato:>

🔴 1. **Capa Comportamiento** [verificada] · `Read definitions/playbooks/compra.yaml`

El playbook instruye X pero la respuesta observada hace Y — la instrucción no cubre este caso. Es la causa del fallo.

⚪ 2. **Capa Routing** · N/A — Petal usa Playbooks, no Flows.

<... capas 3-9 igual: cada una con su marca + fuente + párrafo aparte (o línea compacta si es ⚪ N/A) ...>

**Resumen visual:** <N> 🔴 problema · <N> 🟢 ok · <N> 🟡 supuesta · <N> ⚪ N/A

### Dimensionamiento del bug

<Tabla de las 3 dimensiones + nivel final → 3/5/7 soluciones. Formato en §3.3.>
...
**Reglas clave del MD:**
- **3, 5 o 7 soluciones según dimensionamiento del bug (§3.3)** — rango score 1-10
...
- FORMATO VERTICAL JERÁRQUICO obligatorio en las capas: cada capa con emoji al inicio + título destacado + metadata en línea separada + descripción en párrafo aparte. Las capas N/A van en una sola línea compacta.

### 2.2 El MD de patrones — `qap/tc_analysis/{TS}/_patterns_{TS}.md` (solo BATCH)
...
**Reglas de parseo críticas** (son las que consume `_render_patterns_html` en `test_qa_playbooks.py` — respétalas al carácter):
...
- ROI en formato `N TCs/h` (número entero, no decimal). Cálculo: `(TCs resueltos / minutos_fix) × 60 = TCs/h`
...
**Evaluación de cada patrón** — 3 razones independientes, estados ✅ / ⚠️ / ❌:
...
**Caso sin patrones** — el archivo contiene solo:
```markdown
---
run_ts: {TS}
generado: {FECHA_HOY}
...
**Anotación en cada TC que forme parte de un patrón** — añadir a su sección `## Recomendación`:
...
### 2.3 El reporte al usuario (Paso 7)

**Líneas de retorno de los sub-agentes** (una sola línea, nunca el MD completo):
- Sub-agente TC: `{TC-ID}: OK — {tipo} · fix ~{X}min`
- Sub-agente patrones: `PATRONES: OK — {N_PATRONES} patrón(es) · {N_TCS_CON_PATRON} TCs afectados · HTML publicado`
...
## 3. Referencia: las 9 capas y el scoring

Cada análisis evalúa **OBLIGATORIAMENTE las 9 capas**. Ninguna se omite: si no aplica, se marca ⚪ N/A con justificación.

### 3.1 Las 9 capas

| # | Capa | Cubre | En Petal |
|---|------|-------|----------|
| 1 | **Comportamiento** | Playbooks + Examples + Generators (cómo guía la respuesta del agente). Verificar instrucciones del playbook, examples relevantes al turno, configuración de generadores LLM. | Causa habitual |
| 2 | **Routing** | Flows + Pages + Intents + Entity Types (cómo se enruta la conversación). | Suele ser ⚪ N/A (Petal usa Playbooks, no Flows) |
| 3 | **Parámetros / Slots** | Slots de entrada/salida entre playbooks y entre playbook y tools. Verificar que los parámetros pasan correctamente entre componentes. | Frecuente en transfers |
| 4 | **Integración** | Tools + Webhooks + llamadas API al backend (PetalDataTool). Verificar tool calls, parámetros enviados, respuestas del backend. | — |
| 5 | **Datos** | Sheet (`business` + `agent_copy` + `inventario` + `perfil` + `pedidos`) + coherencia entre recursos. Aquí se reportan los hallazgos de la auditoría del Sheet (§4 Paso 2.5). | — |
| 6 | **Infraestructura** | Environments + Versions + Agent Config. Verificar que el deploy es correcto, que la versión activa es la esperada, que la configuración del agente no introduce regresiones. | — |
| 7 | **Modelo / LLM** | Comportamiento de Gemini — alucinaciones, decisiones no deterministas. **Marcado 🔴 con regla binaria (ver §3.2).** | Último recurso |
| 8 | **Histórico** | Regresiones — `git log` del playbook involucrado (cargado en §4 Paso 2.6). | — |
| 9 | **Test** | Calidad del propio test — regex mal calibrado, caso mal definido, expectativa incorrecta. | Frecuente |

### 3.2 Sistema de marcas

Cada capa lleva emoji + corchete:
...
- ⚪ REQUIERE breve justificación de por qué la capa no aplica. Para capas ⚪ N/A, omitir el párrafo descripción y poner todo en una línea: `⚪ N. **Capa Nombre** · N/A — razón breve.`
- **NUNCA** cites un PR (#N), commit (sha), o variable de negocio sin haberlo verificado con la herramienta correspondiente.
- **SIEMPRE** cierra la sección "Causa raíz" con el **Resumen visual**: `<N> 🔴 problema · <N> 🟢 ok · <N> 🟡 supuesta · <N> ⚪ N/A`

**Regla binaria de la Capa 7 (Modelo / LLM):** se marca 🔴 [verificada] **solo si** las 8 capas restantes son todas 🟢 [verificada] **Y** el bug es reproducible en al menos 2 ejecuciones del mismo TC. En cualquier otro caso, 🟡 [supuesta]. Cuando se marque 🔴, incluir justificación explícita: *"Marcado 🔴 tras descartar las 8 capas restantes (todas 🟢) y confirmar reproducibilidad en N ejecuciones"*.

**Formato vertical jerárquico** de cada capa en el MD:

```markdown
🔴/🟢/🟡/⚪ 1. **Capa Comportamiento** [verificada/supuesta/N/A] · `<fuente o motivo>`

<descripción en párrafo aparte — qué se verificó y por qué esta capa es/no es la causa>
```

### 3.3 Dimensionamiento del bug → 3, 5 o 7 soluciones

En lugar de generar siempre 7 soluciones, se evalúan 3 dimensiones y se generan **3, 5 o 7** según el resultado.
...
**Regla de clasificación:** el nivel final es el **máximo** de las tres dimensiones. Escala siempre hacia arriba, nunca hacia abajo.
- 3 dimensiones en Trivial → **3 soluciones**
- Al menos una en Medio → **5 soluciones**
- Al menos una en Arquitectónico → **7 soluciones**

**Documentar la clasificación en el MD** (bloque `### Dimensionamiento del bug`, antes de "Soluciones evaluadas"):
...
**Determinar TS:**
- Si el usuario nombra TS explícito (`20260518_192907`) → úsalo (tiene prioridad sobre la detección automática)
- Si no → el último de gh-pages:
  ```bash
  curl -s https://jeronimosanchez.github.io/cx-automation-template/qa/ | grep -oE "20[0-9]{6}_[0-9]+" | sort -r | head -1
  ```
...
### Paso 2 — Obtener el JSON del TC (local primero, gh-pages fallback)

**Estrategia:** local primero por velocidad y fiabilidad, gh-pages como fallback.
...
**Tiempos esperados:** lectura local ~0.002 s · fallback gh-pages ~2-3 s.

Si el usuario proporciona los JSONs directamente (pegados o adjuntos), saltarse este paso.

### Paso 2.5 — Auditoría del Sheet (variables de negocio)
...
**3. Reportar:** los hallazgos de coherencia del Sheet se reportan en la **Capa Datos** (capa 5, §3.1). Si la coherencia falla, esa capa puede ser 🔴 incluso si el playbook está correcto.
...
### Paso 2.6 — Cargar fuentes contextuales adicionales
...
**Si hay 1 TC:** analizar inline y escribir el MD directamente con el formato de §2.1. Si el MD ya existe, sobreescribirlo (salvo `--keep-existing`).
...
- El formato exacto del MD (la plantilla completa de §2.1, con la referencia de capas de §3 — **el agente principal la embebe en el prompt del sub-agente**)
...
Lanzar **UN único sub-agente** con contexto limpio usando la herramienta Agent. Este sub-agente asume toda la segunda fase: patrones, HTML, publicación y commit final. **Los Pasos 4, 5 y 6 (patrones, HTML, publicación) están absorbidos por este sub-agente — el agente principal no los ejecuta.**
...
Agrupar TCs por similitud en: misma capa 🔴 · mismo playbook involucrado · mismo síntoma en `agent_text` · mismo fix. Un grupo es patrón si **≥2 TCs** comparten al menos un atributo. Evaluar cada grupo con las 3 razones de §2.2 y calcular el ROI (`(TCs resueltos / minutos_fix) × 60 = TCs/h`).

**PASO C — Escribir `_patterns_{TS}.md`:**
Ruta: `~/cx-automation-template/qap/tc_analysis/{TS}/_patterns_{TS}.md`. Usa la plantilla EXACTA y las **reglas de parseo críticas** de §2.2 (son las que consume `_render_patterns_html` — respétalas al carácter). Si no hay ningún patrón, usa la plantilla del "Caso sin patrones" de §2.2. A cada TC que forme parte de un patrón, añade en su sección `## Recomendación` esta anotación literal:
> **Forma parte del patrón {nombre}.** Si el fix de TC-YYY resuelve la causa raíz común, TC-ZZZ pasará a PASS sin cambios adicionales. Re-ejecutar antes de planificar fixes individuales.
...
Mensaje final adaptado al modo — usar las plantillas de §2.3 (INDIVIDUAL o BATCH).

---

## 5. Reglas, coste y ejemplos
...
Claude:
  ...
  3. Escribe qap/tc_analysis/{TS}/TC-FRUSTRACION-01.md
