"""Tests unitarios para act/push_versions.py.
...
def test_list_flows_paginates():
    ...
    with patch("act.push_versions.requests.get") as mg:
...
def test_create_version_success():
    ...
    with patch("act.push_versions.requests.post") as mp, \
         patch("act.push_versions.requests.get") as mg:
...
def test_create_version_lro_polling_waits_for_done(monkeypatch):
    ...
    monkeypatch.setattr("act.push_versions.time.sleep", lambda _: None)
    with patch("act.push_versions.requests.post") as mp, \
         patch("act.push_versions.requests.get") as mg:
        mp.return_value = _resp(200, {"name": OP_NAME})
        mg.side_effect = [
...
def test_cmd_create_adhoc_uses_default_flow():
    ...
    with patch("act.push_versions.requests.get") as mg, \
         patch("act.push_versions.requests.post") as mp:
        mg.return_value = _resp(200, {"flows": flows})
        mp.return_value = _resp(200, {})
...
def test_delete_version_sync_success():
    with patch("act.push_versions.requests.delete") as md:
...
def test_rotate_no_op_when_under_limit():
    ...
    with patch("act.push_versions.requests.get") as mg, \
         patch("act.push_versions.requests.delete") as md:
