"""Tests unitarios para act/push_playbooks.py.
...
from act.push_playbooks import (
...
def test_list_playbooks_paginates():
    ...
    with patch("act.push_playbooks.requests.get") as mg:
...
def test_upsert_playbook_created_when_missing():
    ...
    existing = {}
    with patch("act.push_playbooks.requests.post") as mp:
...
def test_upsert_playbook_unchanged_when_match():
    ...
    with patch("act.push_playbooks.requests.get") as mg, \
         patch("act.push_playbooks.requests.patch") as mpa, \
         patch("act.push_playbooks.requests.post") as mp:
...
def test_patch_playbook_does_not_send_update_mask():
    ...
    with patch("act.push_playbooks.requests.patch") as mpa:
        mpa.return_value = _resp(200, {})
        patch_playbook(
