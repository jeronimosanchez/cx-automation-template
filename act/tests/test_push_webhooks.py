"""Tests unitarios para act/push_webhooks.py.
...
def test_list_webhooks_paginates():
    ...
    with patch("act.push_webhooks.requests.get") as mg:
...
def test_upsert_webhook_created_when_missing():
    ...
    with patch("act.push_webhooks.requests.post") as mp:
...
def test_upsert_webhook_unchanged():
    ...
    with patch("act.push_webhooks.requests.get") as mg, \
         patch("act.push_webhooks.requests.patch") as mpa, \
         patch("act.push_webhooks.requests.post") as mp:
