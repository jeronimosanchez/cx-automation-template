"""Tests unitarios para act/push_intents.py.
...
def test_list_intents_paginates():
    ...
    with patch("act.push_intents.requests.get") as mg:
...
def test_upsert_intent_created_when_missing():
    ...
    with patch("act.push_intents.requests.post") as mp:
...
def test_upsert_intent_unchanged_when_match():
    ...
    with patch("act.push_intents.requests.get") as mg, \
         patch("act.push_intents.requests.patch") as mpa, \
         patch("act.push_intents.requests.post") as mp:
