...
def record_resource_write(client, project, agent_id, tipo, cx_id, archivo,
    ...
    _sub(client, project, agent_id, SUB_RESOURCES).document(
        ...
        "operacion": operacion,
        "huella_cx": huella_cx,
        # Y cómo queda el **repositorio** tras esta escritura. Con las dos, el
        # diff siguiente sabe cuál de los dos lados se movió y no solo que
        # difieren: cada huella se compara contra su propio pasado. Sin esta,
        # cualquier diferencia se atribuía al repositorio y todo acababa en el
        # Paso 3, incluido lo editado a mano en la consola de CX — que no tenía
        # camino de vuelta.
        "huella_repo": huella_repo,
        "padre": padre,
        "escrito_en": _now(),
    })
