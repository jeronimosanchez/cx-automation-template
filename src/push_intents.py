#!/usr/bin/env python3
"""
src/push_intents.py — Crear/upsert Intents de Dialogflow CX.

Patron `LIST -> diff -> PATCH/POST` (Sprint 3, Task 3). Top-level del
agente, mismo flujo que `push_flows.py`.

Atencion: `trainingPhrases[].parts[]` puede contener tanto texto plano
...
Uso:
  python src/push_intents.py --file=definitions/intents/example_intent.yaml --dry-run
  python src/push_intents.py --all --dry-run
  python src/push_intents.py --all --only Test_Intent_Greeting --dry-run
"""
import argparse
import os
import subprocess
...
INTENTS_DIR = REPO_ROOT / "definitions" / "intents"

INTENT_IGNORE_FIELDS = ["name", "createTime", "updateTime"]
...
def list_intents(cfg, headers):
    ...
    while True:
        ...
        r = requests.get(
            f"{base_url(cfg)}/{parent_path(cfg)}/intents",
            headers=headers, params=params,
        )
        if r.status_code != 200:
            raise RuntimeError(
                f"LIST /intents fallo en pagina {page}: "
                f"{r.status_code} {r.text[:300]}"
            )
        data = r.json()
        items.extend(data.get("intents", []))
...
def get_intent(cfg, headers, name):
...
def create_intent(cfg, headers, body, dry_run=False):
...
def patch_intent(cfg, headers, name, payload, update_mask, dry_run=False):
    short = name.split("/")[-1]
...
def upsert_intent(cfg, headers, body, existing_by_name, dry_run=False):
    remote_summary = existing_by_name.get(body["displayName"])
    if remote_summary is None:
        ok = create_intent(cfg, headers, body, dry_run)
        return "created" if ok else "failed"

    try:
        remote = get_intent(cfg, headers, remote_summary["name"])
    except RuntimeError as e:
        print(f"  ERR {e}")
        return "failed"

    result: DiffResult = diff_resource(
        body, remote, ignore_fields=INTENT_IGNORE_FIELDS,
    )
...
def load_intent_yaml(file_path: Path) -> dict:
...
def discover_intent_files():
    if not INTENTS_DIR.exists():
        return []
    return sorted(INTENTS_DIR.glob("*.yaml")) + sorted(INTENTS_DIR.glob("*.yml"))


def main():
    ap = argparse.ArgumentParser(
        description="Crear/upsert Intents de Dialogflow CX (idempotente).",
    )
    ap.add_argument("--file", help="Path a un YAML de Intent")
    ap.add_argument("--all", action="store_true",
                    help="Procesar todos los YAMLs en definitions/intents/")
    ...
    if args.file:
        ...
        yaml_files = discover_intent_files()
        if not yaml_files:
            print(f"  No hay YAMLs en {INTENTS_DIR}. Nada que hacer.")
            return 0
    ...
    print("\nLIST de Intents remotos del agente (paginado)...")
    try:
        remote_intents = list_intents(cfg, headers)
    except RuntimeError as e:
        print(f"  ERR {e}")
        return 1
    existing_by_name = {i["displayName"]: i for i in remote_intents}
    print(f"   ok {len(remote_intents)} Intent(s) remotos detectados")

    stats = {"created": 0, "updated": 0, "unchanged": 0, "failed": 0}
    mode = "(DRY-RUN)" if args.dry_run else ""
    print(f"\nProcesando {len(bodies)} Intent(s) {mode}...\n")
    for fp, body in bodies:
        print(f"  [{fp.name}]")
        action = upsert_intent(cfg, headers, body, existing_by_name, args.dry_run)
        stats[action] += 1
