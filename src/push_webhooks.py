#!/usr/bin/env python3
"""
src/push_webhooks.py — Crear/upsert Webhooks de Dialogflow CX.

Patron `LIST -> diff -> PATCH/POST` (Sprint 3, Task 5). Top-level del
agente.

Soporta `genericWebService` (caso comun: HTTP backend del cliente).
...
Uso:
  python src/push_webhooks.py --file=definitions/webhooks/example_webhook.yaml --dry-run
  python src/push_webhooks.py --all --dry-run
  python src/push_webhooks.py --all --only Test_Webhook --dry-run
"""
import argparse
import os
import subprocess
...
WEBHOOKS_DIR = REPO_ROOT / "definitions" / "webhooks"

WEBHOOK_IGNORE_FIELDS = ["name", "createTime", "updateTime"]
...
def list_webhooks(cfg, headers):
    ...
    while True:
        ...
        r = requests.get(
            f"{base_url(cfg)}/{parent_path(cfg)}/webhooks",
            headers=headers, params=params,
        )
        if r.status_code != 200:
            raise RuntimeError(
                f"LIST /webhooks fallo en pagina {page}: "
                f"{r.status_code} {r.text[:300]}"
            )
        data = r.json()
        items.extend(data.get("webhooks", []))
...
def get_webhook(cfg, headers, name):
...
def create_webhook(cfg, headers, body, dry_run=False):
...
def patch_webhook(cfg, headers, name, payload, update_mask, dry_run=False):
    short = name.split("/")[-1]
...
def upsert_webhook(cfg, headers, body, existing_by_name, dry_run=False):
    remote_summary = existing_by_name.get(body["displayName"])
    if remote_summary is None:
        ok = create_webhook(cfg, headers, body, dry_run)
        return "created" if ok else "failed"

    try:
        remote = get_webhook(cfg, headers, remote_summary["name"])
    except RuntimeError as e:
        print(f"  ERR {e}")
        return "failed"

    result: DiffResult = diff_resource(
        body, remote, ignore_fields=WEBHOOK_IGNORE_FIELDS,
    )
...
def load_webhook_yaml(file_path: Path) -> dict:
...
def discover_webhook_files():
    if not WEBHOOKS_DIR.exists():
        return []
    return sorted(WEBHOOKS_DIR.glob("*.yaml")) + sorted(WEBHOOKS_DIR.glob("*.yml"))


def main():
    ap = argparse.ArgumentParser(
        description="Crear/upsert Webhooks de Dialogflow CX (idempotente).",
    )
    ap.add_argument("--file", help="Path a un YAML de Webhook")
    ap.add_argument("--all", action="store_true",
                    help="Procesar todos los YAMLs en definitions/webhooks/")
    ap.add_argument("--dry-run", action="store_true",
                    help="No envia POST/PATCH; muestra que se haria")
    ap.add_argument("--only", help="displayName del Webhook a procesar")
    args = ap.parse_args()
    ...
    if args.file:
        ...
        yaml_files = discover_webhook_files()
        if not yaml_files:
            print(f"  No hay YAMLs en {WEBHOOKS_DIR}. Nada que hacer.")
            return 0
    ...
    print("\nLIST de Webhooks remotos del agente (paginado)...")
    try:
        remote_whs = list_webhooks(cfg, headers)
    except RuntimeError as e:
        print(f"  ERR {e}")
        return 1
    existing_by_name = {w["displayName"]: w for w in remote_whs}
    print(f"   ok {len(remote_whs)} Webhook(s) remotos detectados")

    stats = {"created": 0, "updated": 0, "unchanged": 0, "failed": 0}
    mode = "(DRY-RUN)" if args.dry_run else ""
    print(f"\nProcesando {len(bodies)} Webhook(s) {mode}...\n")
    for fp, body in bodies:
        print(f"  [{fp.name}]")
        action = upsert_webhook(cfg, headers, body, existing_by_name, args.dry_run)
        stats[action] += 1
